...
export function makeDateFromDateString(dateString: string): Date {
  ...
  if (strParts.length !== 3)
    throw new UtilError(`Invalid ISO string - invalid length <${dateString}>`);

  const [year, month, day] = strParts.map(Number);
  if (!year || !month || !day)
    throw new UtilError(`Invalid ISO string - invalid content <${dateString}>`);
...
export function makeDateFromISO(iso: string): Date {
  ...
  if (!isoFirstPart)
    throw new UtilError(`Invalid ISO string - missing 'T' <${iso}>`);
...
const DAYS_IN_WEEK: Record<number, string> = {
  0: "Sunday",
  1: "Monday",
  2: "Tuesday",
  3: "Wednesday",
