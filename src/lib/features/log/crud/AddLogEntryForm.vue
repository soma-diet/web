...
import { LogEntryRequestDto, postLogEntry } from '@/lib/api';
import { type Trackable } from '@/lib/model';
...
async function createLogEntry(quantity: number, servingId: string | null, date: Date, callback: () => void) {
  const newEntryRequest = LogEntryRequestDto.createRequest(props.trackable.id, servingId, quantity, date);
